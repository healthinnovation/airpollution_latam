...
bogota <- st_read("Data2/geom/colombia.gpkg") %>% st_transform(crs = 4326)
buenosaires <- st_read("Data2/geom/argentina.gpkg") %>% st_transform(crs = 4326)
guatemala <- st_read("Data2/geom/guatemala.gpkg") %>% st_transform(crs = 4326)
mexico <- st_read("Data2/geom/mexico.gpkg") %>% st_transform(crs = 4326)
panama <- st_read("Data2/geom/panama.gpkg") %>% st_transform(crs = 4326)
sanjose <- st_read("Data2/geom/costarica.gpkg") %>% st_transform(crs = 4326)
santiago <- st_read("Data2/geom/chile.gpkg") %>% st_transform(crs = 4326)
saopaolo <- st_read("Data2/geom/brasil.gpkg") %>% st_transform(crs = 4326)
lima <- st_read("Data2/geom/peru.gpkg") %>% st_transform(crs = 4326)
...
bv_5 <- ggstatsplot::ggbetweenstats(
  ...
  k = 2, # cu?ntos decimales?,
  pairwise.comparisons = TRUE, # Muestra las comparaciones post hoc
  p.adjust.method = "bonferroni",3 # m?todo para utilizar las post hoc. esta: "holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"
  pairwise.display = "s", # "s" solo te muestra las que son significativas, "ns" muestra no significativas, "all" pues todas ;)
  outlier.tagging = TRUE, # mostrar outliers
  outlier.coef = 1.5, # coeficiente para considerarlo outlier siguiendo la Tukey's rule
  xlab = "", 
...
bv_6 <- ggstatsplot::ggbetweenstats(
  ...
  notch = TRUE, # box plot cuadrado o ovalado (notch)
  mean.ci = TRUE, # Intervalos de confianza para las medias
  type = "np", #"p" (para parametrica22), "np" (no parametrica), "r" (robusta), or "bf" (bayes factor)
  effsize.type = "partial_omega",# hay "biased" (equivalente a la d cohen del t test), "partial_eta" (eta-squared para anova) o "unbiased" (equivalente a "g" Hedge's g para t-test, "partial_omega"(omega-squared para anova)
  k = 2, # cu?ntos decimales?,
  pairwise.comparisons = TRUE, # Muestra las comparaciones post hoc
  p.adjust.method = "bonferroni", # m?todo para utilizar las post hoc. esta: "holm", "hochberg", "hommel", "bonferroni", "BH", "BY", "fdr", "none"
...
lm1 <- lm(PM2.5_mean~attending_quantile_c, 
          data = sii1_man)
